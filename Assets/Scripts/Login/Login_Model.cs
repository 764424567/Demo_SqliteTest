...
public class Login_Model
{
    ...
    //带参构造函数
    public Login_Model(string connectionString)
    {
        //打开数据库
    ...
    //查询用户名相同的项
    public SqliteDataReader SelectData(string tableName,string UserName)
    {
        //SQL = SELECT 字段名 FROM 表名 WHERE 列名称 ='值';
    ...
    {
        //SQL = SELECT 字段名 FROM 表名 WHERE 列名称 ='值';
        string query = "SELECT " + items + " FROM " + tableName + " WHERE " + col + " = " + "'" + values + "'";
        return ExecuteQuery(query);
    }
